...
def build_report(result: AnalysisResult) -> str:
    ...
    lines.extend(["", "### 안내", "> 이 결과는 투자자문이 아니라 데이터 기반 후보 진단입니다."])
    return _trim("\n".join(lines), "리포트")
...
def build_recommendation_report(
    ...
    lines = [
        "# 미너비니 종목 추천 후보",
        "### 검색 조건",
        f"- 스캔 대상: **{scanned_count}개**",
        f"- 검증 기준: **{min_score}점 이상** + 핵심 기술 조건 4개 이상 + 유동성 기준 통과",
    ]
    if filter_summary:
        lines.append(f"- {filter_summary}")
    if batch_id:
    ...
    for index, result in enumerate(results, start=1):
        ...
        lines.extend(
            [
                ...
                f"- 등급: **{result.grade}** / 점수: **{result.score}점**",
                f"- 현재가: {price} / 섹터: {sector}",
                f"- 기술 조건: {technical_passed}/6 통과",
                f"- 시가총액: {market_cap} / 50일 평균 거래대금: {dollar_volume}",
                f"> {result.summary}",
                "",
            ]
        )
    ...
    lines.extend(
        [
            "## 기록 안내",
            "- 추천 당시 가격과 점수는 사후 검증을 위해 로컬에 저장됩니다.",
            "- `/미너비니 사후검증`으로 추천 결과를 평가할 수 있습니다.",
            "",
            "### 안내",
            "> 이 결과는 투자자문이 아니라 데이터 기반 후보 선별입니다.",
        ]
...
def build_validation_report(validations: list[ValidationResult]) -> str:
    ...
    for validation in validations[:15]:
        ...
        lines.extend([f"### {validation.ticker} | {status}", f"- {reason_text}"])
    ...
    lines.extend(
        [
            "",
            "### 검증 기준",
            "> 데이터 충분성 · 최소 점수 · 핵심 기술 조건 · 시가총액 · 거래대금 · 최소 주가",
        ]
    )
    return _trim("\n".join(lines), "검증 결과")
...
def build_single_validation_report(result: AnalysisResult, validation: ValidationResult) -> str:
    lines = [
        f"# {result.ticker} 검증 결과",
        "### 판정",
        f"- 결과: **{'통과' if validation.passed else '보류'}**",
    ...
    lines.extend(["", "### 안내", "> 검증 통과는 매수 추천이 아니라 후보군에 올릴 수 있다는 뜻입니다."])
    return "\n".join(lines)
...
def build_performance_report(evaluations: list[RecommendationEvaluation]) -> str:
    ...
    lines.extend(
        [
            "### 요약",
            f"- 평가 대상: **{len(evaluations)}개**",
            f"- 성공: **{success}개** / 관찰: **{watch}개** / 실패: **{failed}개**",
            "",
            "## 종목별 성과",
        ]
    )
    ...
    for item in evaluations[:15]:
        ...
        lines.extend(
            [
                f"### {record.ticker} | {item.verdict}",
                f"- 수익률: **{return_text}** / 경과: {item.days_elapsed}일",
                f"- 추천가: ${record.price:.2f} → 현재가: {current_price}",
                f"- 추천 당시: {record.score}점 / {record.grade}",
            ]
        )
